...
import com.mastodon.Preferences.Preferences;
...
public class SharedPreferencesHelperTest {
    ...
    public void sharedPreferencesHelper_SaveInstallationTime() {
        when(mMockSharedPreferences.getLong(eq(Preferences.PrefKeys.INSTALLATION_TIME.name()), anyLong())).thenReturn(-1L);
        ...
        when(mMockSharedPreferences.getLong(eq(Preferences.PrefKeys.INSTALLATION_TIME.name()), anyLong())).thenReturn(INSTALLATION_TIME);

        // Read is timer expired flag from SharedPreferences
        long installationTime = mMockSharedPreferencesHelper.getInstallationTime();

        // Make sure both written and retrieved personal information are equal.
        assertThat("Checking that the installation timestamp has been persisted and read correctly", installationTime, is(equalTo(INSTALLATION_TIME)));
    ...
    private SharedPreferencesHelper createMockSharedPreference() {
        ...
        // correctly.
        when(mMockSharedPreferences.getString(eq(Preferences.PrefKeys.TOKEN.name()), anyString())).thenReturn(TOKEN);
        when(mMockSharedPreferences.getBoolean(eq(Preferences.PrefKeys.IS_TIMER_EXPIRED.name()), anyBoolean())).thenReturn(IS_TIMER_EXPIRED);
